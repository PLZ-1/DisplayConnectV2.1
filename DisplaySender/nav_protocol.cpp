...
static void copyTruncated(char* dest, size_t destSize, const char* src) {
  ...
  dest[destSize - 1] = '\0';
  // Do not leave a partial UTF-8 character at the end of a bounded text field.
  size_t valid = 0;
  for (size_t i = 0; dest[i];) {
    ...
    for (size_t n = 1; n < bytes; ++n) {
      if (i + n >= destSize || dest[i + n] == '\0' || (static_cast<uint8_t>(dest[i + n]) & 0xC0) != 0x80) { complete = false; break; }
    }
    if (!complete) break;
    i += bytes;
    valid = i;
