...
void showMapLoadingScreen(TFT_eSPI& tft) {
  ...
  tft.drawString(displayEnglish() ? "Getting directions..." : "Calculando rota...", SCR_W / 2, MAP_AREA_H + 28, 2);
  tft.setTextColor(mapsColMuted(), mapsColCard());
  tft.drawString(displayEnglish() ? "Loading map data" : "Carregando mapa", SCR_W / 2, MAP_AREA_H + 52, 2);
