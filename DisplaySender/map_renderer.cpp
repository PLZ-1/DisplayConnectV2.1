...
void MapRenderer::draw(const NavState& state) {
  tft.startWrite();
  tft.setViewport(0, 0, SCR_W, MAP_AREA_H);
  clearMapArea();
  drawStreets(state);
  drawRoute(state);
...
void MapRenderer::drawOverlay(const NavState& state) {
  ...
  const int textMaxW = 312;
  tft.drawFastVLine(332, MAP_AREA_H + 14, 64, mapsColCardShadow());
  tft.setTextColor(mapsColMuted(), mapsColCard());
  tft.drawString(state.english ? "REMAINING" : "RESTANTE", 344, MAP_AREA_H + 12, 1);
  char remainingDistance[24] = "-- km";
  char remainingTime[24] = "-- min";
  if (!state.offRoute && state.remainingM >= 0) {
    snprintf(remainingDistance, sizeof(remainingDistance),
             state.remainingM < 1000 ? "%.2f km" : "%.1f km", state.remainingM / 1000.0);
  }
  if (!state.offRoute && state.remainingS >= 0) {
    const int minutes = state.remainingS / 60 + (state.remainingS % 60 != 0);
    if (minutes >= 60) snprintf(remainingTime, sizeof(remainingTime), "%dh %02dmin", minutes / 60, minutes % 60);
    else snprintf(remainingTime, sizeof(remainingTime), "%d min", minutes);
  }
  tft.setTextColor(mapsColAccent(), mapsColCard());
  const int distanceFont = tft.textWidth(remainingDistance, 4) <= 128 ? 4 : 2;
  tft.drawString(remainingDistance, 344, MAP_AREA_H + 28, distanceFont);
  tft.setTextColor(mapsColText(), mapsColCard());
  drawUtf8Text(tft, state.offRoute ? (state.english ? "Off route" : "Fora da rota") : remainingTime,
               344, MAP_AREA_H + 58, 132, mapsColText(), mapsColCard());
  // Clip long instructions/HTML to the left column so trip totals stay readable.
  tft.setViewport(0, 0, 324, SCR_H);
  ...
  tft.setTextColor(mapsColAccent(), mapsColCard());
  drawUtf8Text(tft, state.gpsWeak ? (state.english ? "Weak GPS" : "GPS fraco") : distLine,
               8, MAP_AREA_H + 14, textMaxW, mapsColAccent(), mapsColCard());

  if (state.instruction[0] != '\0') {
    tft.setTextColor(mapsColText(), mapsColCard());
    /* Keep maneuver text within its column. */
    char instr[128];
    strncpy(instr, state.instruction, sizeof(instr) - 1);
    instr[sizeof(instr) - 1] = '\0';
    drawUtf8Text(tft, instr, 8, MAP_AREA_H + 36, textMaxW, mapsColText(), mapsColCard());
  }
  if (state.street[0] != '\0') {
    tft.setTextColor(mapsColMuted(), mapsColCard());
    drawUtf8Text(tft, state.street, 8, MAP_AREA_H + 58, textMaxW, mapsColMuted(), mapsColCard());
  }
  tft.resetViewport();
}
